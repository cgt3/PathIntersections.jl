...
struct PiecewiseCurve <: # Callable object for piecewise defined curves
    ...
    function PiecewiseCurve(input_stop_pts, input_subcurves, input_s_bounds; continuity_tol=DEFAULT_CONTINUITY_TOL, steps_per_segment=1000)
        ...
        for i = 1:num_curves
            # Update continuity if needed
            if i < num_curves && abs(input_subcurves[i](input_s_bounds[2]) - input_subcurves[i+1](input_s_bounds[1]) ) >= continuity_tol
                is_continuous = false
            end

            # Compute the arc length of this segment
            sub_ds = (input_stop_pts[i][2] - input_s_bounds[i][1]) / steps_per_segment
        ...
        is_closed = false
        if is_continuous == true && abs(input_subcurves[end](input_s_bounds[2]) - input_subcurves[1](input_s_bounds[1]) ) < continuity_tol
            is_closed = true
        end
    ...
    function ds_by_arc_length(C::PiecewiseCurve, step_arc_length::Float; continuity_tol=DEFAULT_CONTINUITY_TOL)
        subfunctions = ConstFunction[]
        for i = 1:length(curve.subcurves)
...
function (curve::PiecewiseCurve)(s::Real)
    ...
    while i < length(stop_pts) && s >= curve.stop_pts[i+1]
        i += 1
    end 
